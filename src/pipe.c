...
uint32_t pipe_cycles = 1;
Pipe_State pipe[4];

void pipe_init(uint8_t cpu)
{
    memset(&pipe, 0, sizeof(Pipe_State));
    pipe.PC = 0x00400000;

    /* 8KB, 4-way associative, 32-byte blocks (64 sets) */
    pipe.icache = cache_new(64, 4, 32);
    /* 64KB, 8-way associative, 32-byte blocks (256 sets) */
    pipe.dcache = cache_new(256, 8, 32);

    /* L2: 256KB, 16-way associative, 32-byte blocks (512 sets)
     * DRAM: 8 banks
     */
    mem_hierarchy_init(512, 16, 32, 8);

    /* branch predictor: 8-bit GHR, 1024-entry direct-mapped BTB */
    pipe.bp = bp_new(8, 1024);
}

void pipe_cycle(uint8_t cpu)
{
#ifdef DEBUG
    printf("\n\n----\n\nPIPELINE (%d):\n", pipe_cycles);
    printf("FETCH: PC = %x\n", pipe.PC);
    ...
    //resolve any memory coming back
    resolve_mem_reqs();
    ...
    uint32_t mem_xtra_cycles;
    uint32_t fetch_xtra_cycles;
    sched_mem_req(&mem_xtra_cycles, &fetch_xtra_cycles);
    pipe.icache_bubbles += fetch_xtra_cycles;
    pipe.dcache_bubbles += mem_xtra_cycles;
    ...
    if (pipe.branch_recover) {
        ...
        if (pipe.icache_bubbles > 0 &&
                (pipe.branch_dest & ~31) != (pipe.PC & ~31))
            pipe.icache_bubbles = 0;
...
void pipe_stage_mem()
{
    ...
    if (pipe.dcache_bubbles) {
#ifdef DEBUG
        printf("dcache stall (%d)\n", pipe.dcache_bubbles);
#endif
        pipe.dcache_bubbles--;
        if (pipe.dcache_bubbles == 0)
        {
            cache_update(pipe.dcache, op->mem_addr & ~3);
#ifdef DEBUG
            printf("dcache fill at %d\n", stat_cycles);
#endif
    ...
    /* access dcache */
    if (op->is_mem) {
        if (!cache_probe(pipe.dcache, op->mem_addr & ~3)) {
#ifdef DEBUG
            printf("dcache miss (%x) at %d\n", op->mem_addr & ~3, stat_cycles);
#endif
            pipe.dcache_bubbles = mem_hierarchy_request(op->mem_addr & ~3, MEM_STAGE);
       //     pipe.dcache_bubbles = 49; /* 49 stall cycles remaining after this one */
            return;
        }
    }
    ...
    /* at this point, the data is in the cache and we can proceed */
...
void pipe_stage_execute()
{
    ...
    if (!op->is_branch && (op->pred_is_branch && op->pred_branch_taken))
        pipe_recover(3, op->pc + 4);
    if (op->is_branch && (!op->pred_is_branch ||
                (op->pred_branch_taken != op->branch_taken) ||
                (op->branch_taken && (op->pred_branch_dest != op->branch_dest)))) {
        /*
        printf("MISP: PC %x taken %d dest %x (pred taken %d pred dest %d pred is_br %d)\n",
                op->pc, op->branch_taken, op->branch_dest, op->pred_branch_taken, op->pred_branch_dest, op->pred_is_branch);
                */

        pipe_recover(3, op->branch_taken ? op->branch_dest : op->pc + 4);
    }

    /* train branch predictor */
    if (op->is_branch || (!op->is_branch && op->pred_is_branch)) {
#ifdef DEBUG
        printf("branch %x: pred %d taken %d\n", op->pc, op->pred_branch_taken, op->branch_taken);
#endif
        bp_update(pipe.bp, op->pc, op->is_branch, op->branch_cond, op->branch_taken, op->branch_dest);
    }
...
void pipe_stage_decode()
{
    ...
    /* remove op from our input and place op in downstream slot */
    pipe.decode_op = NULL;
    pipe.execute_op = op;
}
...
{
    if (pipe.icache_bubbles) {
#ifdef DEBUG
        printf("icache bubble (%d)\n", pipe.icache_bubbles);
#endif
        pipe.icache_bubbles--;
        if (pipe.icache_bubbles == 0 &&
                (!pipe.branch_recover || ((pipe.branch_dest & ~31) == (pipe.PC & ~31)))) {
            cache_update(pipe.icache, pipe.PC);
#ifdef DEBUG
            printf("icache fill at %d\n", stat_cycles);
#endif
        }
    ...
    /* access the icache. */
    if (!cache_probe(pipe.icache, pipe.PC)) {
        pipe.icache_bubbles = mem_hierarchy_request(pipe.PC, FETCH_STAGE);
#ifdef DEBUG
        printf("icache miss (%x) at %d\n", pipe.PC, stat_cycles);
        printf("stall icache for (%d) cycles\n", pipe.icache_bubbles);
#endif
        return;
    }

    /* if we reach here, we had a cache hit. Allocate an op and send it down
     * the pipeline. */
    ...
    uint8_t branch, cond, taken;
    uint32_t dest;
    bp_predict(pipe.bp, pipe.PC, &branch, &cond, &taken, &dest);

    op->pred_is_branch = branch;
    op->pred_is_cond = cond;
    op->pred_branch_dest = dest;
    op->pred_branch_taken = taken;

    uint32_t nextPC = (branch && (!cond || taken)) ? dest : (pipe.PC + 4);
